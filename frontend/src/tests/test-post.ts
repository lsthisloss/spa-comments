...
function main(usersCount: number, postsPerUser: number) {
  console.log(`Starting test with ${usersCount} users, ${postsPerUser} posts each. Socket URL: ${SOCKET_URL}`);
  ...
  for (let i = 0; i < usersCount; i++) {
    ...
    userSocket.on('connect', () => {
      console.log(`Connected to /users namespace for user ${userData.userName}`);
      ...
      userSocket.emit('register', userData, (registerResponse: {
        ...
        console.log(`User ${userData.userName} registered:`, registerResponse);
        ...
        if (registerResponse.success && registerResponse.token && registerResponse.user?.id) {
          ...
          postSocket.on('connect', () => {
            console.log(`Connected to /posts namespace for user ${userData.userName}`);
            ...
            for (let j = 0; j < postsPerUser; j++) {
              setTimeout(() => {
                ...
                postSocket.emit('addPost', postDto, (postResponse: { 
                  ...
                  console.log(`Post #${j + 1} for user ${userData.userName}:`, postResponse);
                });
          ...
          postSocket.on('connect_error', (error: Error) => {
            console.error(`Posts socket connection error for ${userData.userName}:`, error);
          });

          postSocket.on('disconnect', (reason: string) => {
            console.log(`Disconnected from /posts for user ${userData.userName}. Reason:`, reason);
          });
        } else {
          console.error(`Registration failed for ${userData.userName}:`, registerResponse);
        }
    ...
    userSocket.on('connect_error', (error: Error) => {
      console.error(`Users socket connection error for ${userData.userName}:`, error);
    });

    userSocket.on('disconnect', (reason: string) => {
      console.log(`Disconnected from /users for user ${userData.userName}. Reason:`, reason);
    });
